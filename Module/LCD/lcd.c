...
void lcd_init(void)
{
    ...
    DWT_Delay_ms(50); /* 初始化FSMC后,必须等待一定时间才能开始初始化 */
...
void drawCurve(float rawValue)
{
    uint16_t x, y;

    if (rawValue > autoScale) { autoScale = rawValue; } // 记录最大值
        if (autoScale < 1) { autoScale = 1; } // 防止除0
        if (autoScale > 8) { autoScale = 8; } // 防止超出屏幕高度
  

    y = 400 - (rawValue / autoScale) * 2; // data processing code (rawValue ) / 200 < autoScale

    if (y > LCD_HEIGHT) y = LCD_HEIGHT; // 防止超出屏幕高度
    if (y < 1) y = 1;                   // 防止超出屏幕高度
    // 这里之所以是120-rawValue/280，与屏幕的扫描方向有关，如果出现上下颠倒的情况，可以改成120 +
    if (firstPoint) // 如果是第一次画点，则无需连线，直接描点即可
    {
        lcd_draw_point(0, y, BLUE);
        lastX      = 0;
        lastY      = y;
        firstPoint = 0;
    } else {
        x = lastX + 1;
        if (x < LCD_WIDTH) // 不超过屏幕宽度
        {
            lcd_draw_line(lastX, lastY, x, y, BLUE);
            lastX = x;
            lastY = y;
